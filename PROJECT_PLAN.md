...
flowchart TD
  ...
  CF --> BE[FastAPI backend<br/>interface/chat_ws.py<br/>WS frames: thinking/token/tool_call/response/error/stopped]
...
never blocked for one-off user messages — enforcement waits for measured
false-positive rates. **Stored artifacts get a stricter gate**: persona
`system_prompt` and a user's `custom_instructions` are replayed on every
future turn, so `update_me`/`create_persona` run the same pattern floor at
WRITE-time and REJECT (`400`) on a match — a different risk profile than a
one-off message. **Retrieved documents are screened too**: beyond the
existing `wrap_untrusted` fencing (which neutralizes an embedded
instruction either way), each excerpt block is separately screened at
read-time and tagged `guard:doc_injection_suspected` if it contains
override-style phrasing — visibility that an attempt was made, not just
that it failed. Full OWASP-LLM Top-10 disposition: `SECURITY_REVIEW.md`.

### 15.1 RAG Security & Quality Hardening Playbook (generalizable — reuse for any future retrieval-augmented agent)

A durable reference, not a finzorr changelog entry: the 12-point RAG
security/quality checklist, each row stating the GENERIC mitigation pattern
first (reusable verbatim when designing a new agent) and then how finzorr
concretely implements it today. The Status column is deliberately honest —
"Covered" means a real enforcement point exists; "Partial" means a
deterministic/pattern-floor signal exists but isn't semantic verification;
"Deferred" means designed but intentionally not yet built, with the reason
stated. Two rows beyond the original 12 (13–14) are my own additions —
real-world RAG-agent quality practices the checklist didn't name but a
production agent needs.

| # | Risk | Generic mitigation pattern | How finzorr implements it | Status |
|---|---|---|---|---|
| 1 | **Retrieval hardening** | Re-rank retrieved candidates for relevance AND diversity, not just top-score; guard against one lucky match dominating the whole context window; cap what's fed downstream. | Maximal Marginal Relevance re-ranking (`vector_store.py::_mmr_select`, pure-cosine, no new dependency, `RAG_MMR_LAMBDA` tunable) fetches `top_k*2` and diversity-selects `top_k`, always keeping the rank-1 hit as a floor; full-document expansion gated on ≥2 same-doc hits OR explicit whole-document intent, not a single scoring chunk (`rag.py::_expand_best_small_doc`); deterministic retrieval hit-rate eval, local release gate (`evals/rag_eval.py`, 15/15 recorded). | Covered |
| 2 | **Prompt injection** | Treat ALL external/retrieved content as untrusted DATA, fenced and never followed as instructions; screen for override phrasing at both read (retrieved content) and write (stored, replayed artifacts) boundaries — a stored artifact needs a stricter gate than a one-off message. | `wrap_untrusted` fencing on every external source; `guard.screen_floor` pattern floor + optional LLM tier; write-time `400` reject on persona/custom-instructions (`auth.py`, `sharing.py`); read-time `guard:doc_injection_suspected` tag on retrieved excerpts (`rag.py`); 76-check injection eval gates CI at 100%. | Covered (fencing + write-time reject); Partial (read-time doc screen — observe-only signal, not a block) |
| 3 | **Access control** | Build retrieval filters SERVER-SIDE from the authenticated session only — never client-supplied — and seam-test that a refactor can't silently widen them. | Tenant-partitioned Qdrant payloads, `["glossary", user_id]` filter built server-side, seam-tested (`tests/test_rag_isolation.py`, incl. the dev pseudo-user); REST ownership checks on every list/read/delete; right-to-erasure across both backends. | Covered |
| 4 | **Data security (PII)** | Detect PII at ingest as TYPE metadata only, never store/log the matched value; redact only what LEAVES the system boundary (traces/telemetry) via a copy — never mutate what's stored or sent to the model itself, since the model needs the real content to be useful and access control is the real cross-tenant boundary, not self-redaction. | `app/domain/pii.py` (`detect_pii`/`redact_for_trace`, anchored regex for email/phone/PAN/Aadhaar-shaped/card-shaped/IFSC); ingest-time PII-type tagging on the trace; LangSmith trace redaction proven byte-identical to the real LLM call via a dedicated test (`dataclasses.replace` builds new message objects — never in-place mutation of the objects `provider.chat()` receives). | Covered |
| 5 | **Hallucination control** | A cheap deterministic floor (citation presence) runs on every turn; a periodic LLM-judge faithfulness score is the real semantic layer — never conflate the two, and don't pay judge-call cost on every turn. | `hallucination:no_citations` tag when hits exist but the answer carries zero `[n]` markers (`rag.py`); `evals/rag_eval.py --judge` faithfulness axis (8.2/10 recorded, `qwen2.5:14b`) as the semantic layer, enforced via `release_gate.sh` per release, not per turn. | Partial — deterministic/pattern-floor signal, not semantic verification |
| 6 | **Context validation** | Validate that what got retrieved is actually proportionate and usable before it's substituted into context — bounded size, not a wholesale dump triggered by a single weak match. | G1's multi-hit-OR-intent gate before full-document expansion; 12k char cap on expanded text; zero-hit path stays an honest refusal (tested — no false hallucination tag on the honest-refusal path). | Covered |
| 7 | **Output validation** | Screen generated text for failure classes that are cheap to detect deterministically — leaked secrets, echoed system prompt, degenerate/repetitive output — before or alongside delivery; stay observe-only until false-positive rates are measured on real traffic. | `guard.screen_output` (secret-shaped regex, system-prompt-fragment echo, degenerate-output heuristic) wired into `rag_node`/`compose_node`, tag-only, never mangles the answer. | Partial — deterministic/pattern-floor signal, not semantic verification |
| 8 | **Citation validation** | After generation, mechanically verify every citation marker actually maps to a real retrieved source in range — never trust the model's own citation discipline unchecked. | `app/domain/citations.py::find_invalid_markers`, wired into `rag_node` and `compose_node`, `citation:invalid` tag on a dangling marker; `evals/grounded_eval.py::_eval_rag` closes the matching eval-side gap. | Covered (mechanical range-check); faithfulness axis (row 5) is the semantic complement |
| 9 | **Prompt hardening** | Distinguish risk profiles: a one-off user message can stay observe-only, but a STORED artifact replayed on every future turn (system prompt, persona, saved instructions) needs write-time rejection — never let user-controlled text silently become part of the system prompt without a trust boundary. | `update_me`/`create_persona` reject (`400`) `custom_instructions`/`system_prompt` matching the guard's jailbreak floor before saving. | Covered (write-time); read-time one-off messages stay Partial by design (observe-only) |
| 10 | **Vector DB security** | The vector store is a real network service — require authentication, bind its port no wider than actually needed, keep tenant partitioning server-side (not client-supplied). | Dev Qdrant requires `QDRANT__SERVICE__API_KEY` and binds `127.0.0.1` only — live-verified 401 unauthenticated / 200 authenticated / no non-loopback port; production posture (tunnel-only ingress) is DESIGNED in `DEPLOYMENT_PLAN.md`, the deploy compose artifact intentionally not yet built (an explicit, documented scoping decision from an earlier session turn, not an oversight). | Covered (dev); Deferred (prod compose artifact — documented, not silently missing) |
| 11 | **Monitoring** | Two genuinely different signals need two watchers: did our deterministic checks regress versus a prior run (eval-based — catches code/prompt regressions), and is LIVE traffic actually degrading right now (traffic-based — catches model/provider drift no eval run can see). | `scripts/drift_watch.py` (daily, eval-based, ALERT+exit-1 on any regression vs. the previous run, tracing pinned OFF inside so the habit itself can't leak) + `scripts/trace_health_watch.py` (NEW — queries live LangSmith for the `degraded`/`guard:suspicious` tag RATE over a trailing window, ALERT+exit-1 on threshold breach, skips gracefully when tracing is off — the inverse credential contract of drift_watch, intentionally, since this script's whole purpose depends on live LangSmith access). | Covered |
| 12 | **Evaluation** | One command should chain every gate — lint, types, tests, deterministic evals, retrieval hit-rate, faithfulness judge — so "did I run everything before shipping" isn't a manual checklist a release can silently skip. | `scripts/release_gate.sh`: mypy, ruff, full pytest, `plan_eval`, `injection_eval`, `routing_eval`, `rag_eval --min-hit-rate 0.9 --judge --min-score 7`, `grounded_eval`, chained with exit-on-first-failure; documented as THE pre-release command (§22). | Covered (as a local gate — CI has no Qdrant/Ollama, a deliberate scope, not a gap) |
| 13 | **Cost & token governance** *(added)* | Cap spend per user and per time window; degrade gracefully across a provider chain rather than hard-fail when a budget is hit; make token usage visible per-trace so cost drift is diagnosable, not just billable. | Per-user Redis sliding-window rate limit + global per-provider daily token budget with automatic fall-through down the free provider chain (`app/infrastructure/llm/completion.py`); `usage_metadata` (`ls_provider`/`ls_model_name`) on every `llm` trace run. | Covered |
| 14 | **Auditability & human feedback loop** *(added)* | Every generated answer should be traceable back to its exact inputs/retrieval/model version; user feedback should close the loop into the eval/golden-dataset pipeline, not evaporate into a ratings table nobody reads. | LangSmith root-run-id pre-assigned and persisted on the `Message` row; 👍/👎 fires `create_feedback` onto the exact trace (fire-and-forget, never blocks); thumbs-down rows are the stated source for new golden-dataset items (§16). | Covered (feedback→trace link); Partial (thumbs-down→golden-dataset promotion is a manual/weekly process today, not automated) |
...
eval/drift runs so daily habits can't leak traces. **Live-traffic health
watch:** `scripts/trace_health_watch.py` — a genuinely different signal
from drift_watch's eval-based checks — queries LangSmith for the RATE of
`degraded` and `guard:suspicious` tags over a trailing window of REAL
traffic, alerts (ALERT + exit 1) on threshold breach, and skips gracefully
(exit 0) when tracing is off/unconfigured; unlike drift_watch it
deliberately does NOT zero the LangSmith env vars — its entire purpose
depends on live credentials being present. See §15.1 row 11 for the
generalized monitoring pattern this pair implements.
...
|---|---|---|
| Orchestrator graph | `backend/app/orchestration/graph.py` | LangGraph `StateGraph`: supervisor → 1 of 6 specialist nodes (`backend/app/specialists/`) → persist |
| Supervisor/router | `backend/app/orchestration/supervisor.py` | One LLM call → JSON `{route, plan, reason}` + deterministic regex keyword fallback (works with the LLM down; unit-tested) |
| Conversation state | `backend/app/orchestration/turn.py` + checkpointer | LangGraph `AsyncPostgresSaver`, `thread_id = session_id` — history survives restarts; graceful degrade to DB-reloaded history |
| LLM gateway | `backend/app/infrastructure/llm/` | One `OpenAICompatibleProvider` class (openai SDK, swappable base URL): Ollama `qwen2.5:14b` in dev; Groq/Gemini/OpenRouter/HF via env keys. Bounded fallback retry + per-provider daily token budgets (Redis) |
| Streaming | `backend/app/interface/chat_ws.py` | FastAPI WebSocket — thinking/token/tool_call/response/stopped/error frames; mid-stream cancel works because turns run as background asyncio tasks; manual Origin validation; cookie auth at handshake |
...
| Image understanding | `backend/app/infrastructure/llm/vision.py` + `interface/attachments.py` | Base64 image → Gemini flash (if key) or local Ollama vision model (`VISION_MODEL`); PNG/JPEG magic-byte validation, ≤5MB |
...
| Share links / personas | `backend/app/interface/sharing.py` | `share_tokens` (public read-only transcript endpoint) + `personas` (per-session system-prompt overlay injected in `turn.py`) |
...
| Feedback loop | `interface/chat.py` + `models/feedback.py` | 👍/👎 → `feedback` table with route/query/response/citations — the future eval golden-dataset seed |
...
- **Before every release wave (local):** `uv run python -m evals.rag_eval
  ...
  15/15 retrieval (100%), faithfulness 9.4/10 (qwen2.5:14b, 2026-08-09 —
  up from 8.2 pre-MMR, same run that validated §15.1's G5).
- **THE single pre-release command:** `backend/scripts/release_gate.sh` —
  chains mypy, ruff, the full pytest suite, `plan_eval`, `injection_eval`,
  `routing_eval`, `rag_eval --min-hit-rate 0.9 --judge --min-score 7`, and
  `grounded_eval`, exiting on the first failure. Supersedes running the
  above gates individually — this is what actually gets run before a
  release, not a checklist that can be silently skipped. First full
  end-to-end run (2026-08-09): mypy clean (158 files), ruff clean, 375
  tests passed, routing 94.0%, injection 76/76, plan mechanics 25/25, rag
  retrieval 15/15 + judge 9.4/10, groundedness 4/4 clean — all green.
- **Daily (cron):** `uv run python scripts/drift_watch.py` — alerts on any
  eval regression versus the previous run.
- **Weekly-or-as-needed (cron, needs live LangSmith credentials):** `uv run
  python scripts/trace_health_watch.py` — alerts on live-traffic
  `degraded`/`guard:suspicious` tag rate exceeding threshold; skips cleanly
...
### Wave 6 — RAG Security & Quality: all 12 checklist points closed solid (§15.1 Playbook)

64. **Every point of the user's 12-item RAG security/quality checklist made solid, plus 2 self-identified additions — codified as a reusable Playbook (§15.1), not just a changelog entry.** *What:* an audit against actual code (not docs) found 3 of 12 fully covered, 5 partial, 4 real gaps — including two doc/code mismatches (`SECURITY_REVIEW.md` overclaiming Qdrant port safety and trace scrubbing). All 12 were closed or made honestly Partial, in dependency order after two adversarial plan reviews: **G3 — data security** (`app/core/pii.py`: `detect_pii`/`redact_for_trace`, anchored regex for email/phone/PAN/Aadhaar-shaped/card-shaped/IFSC; ingest-time PII-type tagging on the trace; LangSmith redaction proven byte-identical to the real LLM call via a dedicated test — `dataclasses.replace` builds new message objects, never in-place mutation of what `provider.chat()` receives). **G4 — context validation** (fixed a real live bug: `_expand_best_small_doc` substituted a whole small document into context off a SINGLE lucky chunk match; now gated on ≥2 same-doc hits OR explicit whole-document intent, anchored regex to avoid reintroducing a milder false-positive). **G1 — citation validation** (`app/core/citations.py::find_invalid_markers`, the exact range-check logic already proven in `grounded_eval.py`, now extracted to run at RUNTIME too — wired into `rag_node` and `compose_node`, `citation:invalid` tag, text never mangled). **G6 — hallucination control** (`hallucination:no_citations` tag when hits exist but the answer carries zero `[n]` markers — a cheap deterministic floor above the existing 8.2/10 faithfulness judge). **G7 — prompt hardening** (write-time: `update_me`/`create_persona` reject `400` on jailbreak-pattern instructions before saving a STORED, replayed artifact — a stricter gate than the existing observe-only message screen; read-time: retrieved document excerpts separately screened and tagged `guard:doc_injection_suspected`, on top of the existing `wrap_untrusted` fencing). **G2 — output validation** (new `guard.screen_output`: secret-shaped strings, verbatim system-prompt echo, degenerate output — wired into `rag_node`/`compose_node`, tag-only). **G5 — retrieval hardening** (Maximal Marginal Relevance re-ranking in `vector_store.py::search` — pure-python cosine, no new dependency, `top_k*2` fetched with vectors and diversity-selected down, always keeping the rank-1 hit as a floor; a `mmr: bool` parameter keeps `app/memory/facts.py`'s every-turn recall on its original single-fetch cost profile, branched BEFORE the query, not just at selection). **G8 — vector DB security** (dev Qdrant now genuinely requires `QDRANT__SERVICE__API_KEY` and binds `127.0.0.1` only — live-verified 401 unauthenticated / 200 authenticated / no non-loopback port; corrected `SECURITY_REVIEW.md`'s prior overclaim; production posture stays documented-not-built in `DEPLOYMENT_PLAN.md`, an explicit prior scoping decision, not silently reopened). **G9 — monitoring** (new `scripts/trace_health_watch.py`: queries live LangSmith for the `degraded`/`guard:suspicious` tag RATE over a trailing window — a genuinely different signal from the existing eval-based `drift_watch.py` — ALERT+exit-1 on threshold breach, skips gracefully when tracing is unconfigured; deliberately does NOT copy `drift_watch.py`'s credential-zeroing lines, since this script's entire purpose depends on live LangSmith access). **G10 — evaluation** (no CI change — confirmed `rag_eval`/`grounded_eval` genuinely can't run in hosted CI, no Qdrant/Ollama service; `scripts/release_gate.sh`, new this wave, is the real enforcement mechanism: chains mypy, ruff, full pytest, `plan_eval`, `injection_eval`, `routing_eval`, `rag_eval --min-hit-rate 0.9 --judge --min-score 7`, `grounded_eval`, exit on first failure — documented as THE pre-release command, §22). Plus **§15.1**, a new generalizable Playbook table (risk → generic pattern → finzorr implementation → honest status) meant to be reused when designing any future retrieval-augmented agent, with 2 self-identified additions beyond the original 12: **cost & token governance** (already-built per-user rate limits + daily token budget + free-provider-chain fallback, now named as its own row) and **auditability & feedback loop** (the existing pre-assigned-run-id + `create_feedback` mechanism, now named and honestly scoped — thumbs-down→golden-dataset promotion is manual today, not automated). *Why:* the user's explicit ask was to make every checklist point genuinely solid (not just documented), add anything missing from my own expertise, and — critically — durably codify it in `PROJECT_PLAN.md` as a reusable reference for designing FUTURE agents, reviewed until solid. *How:* two adversarial plan reviews before a line of code — round 1 caught a trace-redaction aliasing bug (would have silently corrupted what the LLM actually sees) and a guard truthy-string bug (would have rejected every save) before either shipped; round 2 caught a facts.py cost-profile regression (MMR's oversized fetch would have added permanent per-turn overhead to every-turn recall) and a G4 signature change that would silently break two passing tests, both fixed in the same commit as the source change. Execution in file-conflict-aware dependency order (G3 → G4 → G1+G6+G7+G2 as one coordinated `rag_node` diff → G5 → G8 → G9 → docs). Suite grew to 311 sanity tests; mypy `--strict` and ruff clean on every touched file; G8 and G9 both live-verified against the real dev stack (Qdrant 401/200/loopback; `trace_health_watch.py` querying the real LangSmith project and returning a real rate).

---

*Document generated 2026-08-05, maintained through six review-and-improve waves (last update: RAG Security & Quality Playbook, §15.1). Source-of-truth references: `enterprise-assistant-architecture.drawio` (14-page reference architecture) and the code-verified `local-agent-platform` Assistant Pro implementation (prior art, read-only).*
